...
### Fixed
- "Watches by Year" on the Dashboard now groups by the year you actually
  watched a movie/show, not its release/air year. Matches the "year in
  review" convention used by Trakt/Letterboxd.
- The "Watches by Year" chart no longer stretches to fill most of the screen
  when there are only a few bars (e.g. a small watch history).
...
### Fixed
- Translation placeholders now render correctly in paginated movie lists.

### Tests
- Added regression coverage for translated placeholders and plural arguments.
